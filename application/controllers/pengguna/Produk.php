...
class Produk extends MY_Controller
{
    ...
    public function index(){
        ...
        $this->template->load('layout/template', 'pengguna/produk_index', array_merge($data,$data2));
    }
        ...
        if($_FILES['foto']['size'] >= 500 * 1024){
            $this->session->set_flashdata('alert', '
                ...
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    ...
                    ');
            redirect('pengguna/produk/foto/'.$this->input->post('kode_produk'));  
    ...
    public function delete_data($kode_produk){
    ...
    public function delete_foto($namafile,$kode_produk){
        ...
        $this->session->set_flashdata('alert', '
            ...
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                ...
                ');
        redirect('pengguna/produk/foto/'.$kode_produk);
    }
    public function editdata($id){
        ...
        $data = array(
            ...
            'site'                  => $site,
            'nav'                   => '
                    <a class="navigasi-link" href="../"> Produk</a>
                    &nbsp; / &nbsp; <a class="navigasi-link"> <b> <i>Perbarui Produk</i></b></a>
                    &nbsp; / &nbsp; <b> <i>'.$id.'</i></b>
            '
        );
        $this->db->select('*')->from('kategori');
        ...
        $this->template->load('layout/template', 'pengguna/produk_edit', array_merge($data, $data2));
    }
    public function foto($id){
        ...
        $data = array(
            ...
            'nav'                   => '
                    <a class="navigasi-link" href="../"> Produk</a>
                    &nbsp; / &nbsp; <a class="navigasi-link"> <b> <i>Foto Produk</i></b></a>
                    &nbsp; / &nbsp; <b> <i>'.$id.'</i></b>
            '
        );
        $this->db->select('*')->from('kategori');
        ...
        $where = array('kode_produk' => $id);
        $data2['produk'] = $this->CRUD_model->edit_data($where,'produk')->result();
        $this->template->load('layout/template', 'pengguna/produk_foto', array_merge($data, $data2));
    }
